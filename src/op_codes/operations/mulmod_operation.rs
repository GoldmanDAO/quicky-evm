...
#[derive(Clone)]
pub struct MulModOperation {}

impl CodeOperation for MulModOperation {
...
mod tests {
    ...
    fn test_mulmod_operation() {
        let addmod_op = MulModOperation {};
        let stack: Vec<Vec<u8>> = vec![vec![0x03], vec![0x05], vec![0x0f]]; // numbers 3 and 5, modulus 15
        let mut vm = ExecutionRuntime::new_with_stack(stack);
        addmod_op.execute(&mut vm, None);
        assert_eq!(vm.stack, vec![vec![0x0]]); // (3 * 5) % 15 = 0
    }
}
